// Local Codex usage extension for Pi.
// Based on @narumitw/pi-codex-usage from narumiruna/pi-extensions (MIT).
// Local customizations should live here instead of editing node_modules.
...
export default function localCodexUsage(pi: ExtensionAPI) {
	...
	const setUsageStatusline = (
		...
		if (statuslineClearTimer) clearTimeout(statuslineClearTimer);
		statuslineClearTimer = undefined;
		const text = formatCodexUsageStatusline(report, options.model);
		ctx.ui.setStatus(STATUS_KEY, colorCodexUsageStatusline(ctx, report, options.model, text));
...
function colorCodexUsageStatusline(
	...
	const snapshot = selectSnapshotForModel(report, model);
	const usedPercents = [snapshot?.primary?.usedPercent, snapshot?.secondary?.usedPercent].filter(
		(value): value is number => typeof value === "number" && Number.isFinite(value),
	);
	const maxUsedPercent = usedPercents.length > 0 ? Math.max(...usedPercents) : 0;
	if (maxUsedPercent > 90) return ctx.ui.theme.fg("error", text);
	if (maxUsedPercent > 70) return ctx.ui.theme.fg("warning", text);
	return ctx.ui.theme.fg("dim", text);
}
